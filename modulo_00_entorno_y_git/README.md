...
| `ls` | Lista los archivos y carpetas del directorio actual. | `ls -la` *(muestra archivos ocultos y detalles)* |
...
| `rm` | Elimina un archivo o carpeta. | `rm archivo.txt` o `rm -rf carpeta/` |
...
3. **`git commit -m "mensaje explicativo"`**: Guarda una "foto" o versión inmutable de los cambios preparados con un mensaje descriptivo.
...
2. Navegá hasta la carpeta del Módulo 00:
   ```bash
   cd modulo-00-entorno-y-git
...
1. Escribí tu nombre dentro del archivo usando la consola:

```
echo "Hola, soy [Tu Nombre] y estoy aprendiendo Data Engineering" &gt; practica.txt

```

1. Verificá que el contenido se guardó correctamente:
...
1. Guardá los cambios en Git y súbelos a GitHub:

```
git add practica.txt
git commit -m "docs: agregar archivo de practica del Modulo 00"
